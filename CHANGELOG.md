...
- **Type system & evaluation.** Immutable typed values, operators (incl. temporal
  arithmetic), transformer methods, object/array access, lazy `and`/`or`/`??`/ternary and
  desugared `match`; suspendable `Ok | Susp | Err` evaluator with full re-evaluation.
- **Static analysis.** `validate` reports `UNDECLARED_NAME`, `UNKNOWN_FUNCTION`,
  ...
  `UNKNOWN_CAPABILITY`, `POLICY_FORBIDDEN` (incl. `allowedTypes`/`allowedFunctions`), backed
  by a conservative static type inferencer. `analyze` computes requirements, graph, execution
  plan, capabilities, static values, determinism, streamability and metrics.
...
- Intentionally out of scope for v1: a shipped `fake.*` library, `steboStream` streaming,
  and the `lazyParse`/`stream`/`objectPool` optimizations (accepted but inert).
